...
class MemSegDataset(Dataset):
    def __init__(
        ...
        self.target = target
        self.file_list = glob.glob(os.path.join(self.datadir, self.target, 'train/good/*' if train else 'test/*/*'))
        ...
        for fp in self.file_list:
            ...
            if train:
                foreground = cv2.imread(file_path.replace('good', 'foreground'), cv2.IMREAD_GRAYSCALE)
    ...
    def generate_target_foreground_mask(self, img: np.ndarray) -> np.ndarray:
        ...
        if self.target in ['carpet', 'cable', 'wood']:
            target_foreground_mask = np.ones(img.shape[:2])
        elif self.target in ['toothbrush', 'hazelnut', 'metal_nut', 'pill']:
