...
from .utils import FPN, build_position_encoding, nonlinearity, Normalize, make_attn, Backbone_ResNet
...
class VQModel(pl.LightningModule):
    def __init__(self,
        ...
        self.quantize = nn.ModuleList([VectorQuantizer(n_embed, embed_dim * fold ** 2, beta=0.25, remap=remap, sane_index_shape=sane_index_shape) for n_embed, embed_dim, fold in zip(n_embeds, embed_dims, folds)])

        self.shapes = [int(image_size / 2 ** i) for i in range(2, 6)]
        self.folds = folds
        self.unfolders = [nn.Unfold(kernel_size=f, stride=f, padding=0, dilation=1) for f in folds]
        self.folders = [nn.Fold(output_size=(s, s), kernel_size=f, stride=f, padding=0, dilation=1) for s, f in zip(self.shapes, folds)]
        ...
        # self.get_pos_embed = nn.ModuleList([build_position_encoding(pos_dim) for pos_dim in pos_dims])

        # self.channels = [embed_dim - pos_dim for (pos_dim, embed_dim) in zip(pos_dims, embed_dims)]
    ...
    def unfold(self, features):
        features_unfold = []
        shapes = []
        for i, f in enumerate(features):
            B, C, H, W = f.shape
            features_unfold.append(self.unfolders[i](f).reshape(B, -1, H//self.folds[i], W//self.folds[i]))
            shapes.append((H, W))
    ...
    def encode(self, x):
        ...
        encoder_features_unfold = self.unfold(encoder_features)
        
        encode_info = [self.quantize[i](h) for i, h in enumerate(encoder_features_unfold)]
        return encoder_features, encoder_features_unfold, encode_info
    ...
    def forward(self, input):
        encoder_features, encoder_features_unfold, encode_info = self.encode(input)
        quants = [self.folders[i](info[0].flatten(2)) for i, info in enumerate(encode_info)]
        diffs = [info[1] for info in encode_info]

        decoder_features, dec = self.decode(quants)
        # decoder_features, dec = self.decode(encoder_features)
        decoder_features_unfold = self.unfold(decoder_features)
        ...
        # for efu in encoder_features_unfold:
        #     print(efu.shape)
        ...
        # for dfu in decoder_features_unfold:
        #     print(dfu.shape)
        
        # print("====================")

        return dec, diffs, encoder_features, decoder_features, encoder_features_unfold, decoder_features_unfold, quants 
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        x = batch
        xrec, qloss, encoder_features, decoder_features, encoder_features_unfold, decoder_features_unfold, quants = self(x)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        xs, masks = batch
        xrec, qloss, encoder_features, decoder_features, encoder_features_unfold, decoder_features_unfold, neighbor_features = self(xs)

        def dissimilarity(m1, m2):
            return 1 - torch.cosine_similarity(m1, m2, dim=1).unsqueeze(1)
        
        
        # preds = (F.interpolate(dissimilarity(decoder_features[-1], encoder_features[0]), scale_factor=4, mode='bilinear') + \
        #             F.interpolate(dissimilarity(decoder_features[-2], encoder_features[1]), scale_factor=8, mode='bilinear') + \
        #             F.interpolate(dissimilarity(decoder_features[-3], encoder_features[2]), scale_factor=16, mode='bilinear')).squeeze().detach().cpu().numpy()   # [B, 1, H, W]
        
        
        preds = (F.interpolate(dissimilarity(decoder_features_unfold[0], encoder_features_unfold[0]), scale_factor=16, mode='bilinear') + \
                    F.interpolate(dissimilarity(decoder_features_unfold[1], encoder_features_unfold[1]), scale_factor=16, mode='bilinear') + \
                    F.interpolate(dissimilarity(decoder_features_unfold[2], encoder_features_unfold[2]), scale_factor=16, mode='bilinear')).squeeze().detach().cpu().numpy()   # [B, 1, H, W]
        
        preds = np.stack([ndimage.gaussian_filter(p, sigma=4) for p in preds], axis=0)
        res = {"preds": preds, "masks": masks.cpu().numpy()}
    ...
    def configure_optimizers(self):
        ...
        # params = list(self.encoder.parameters()) + list(self.decoder.parameters()) + list(self.fpn.parameters())
    ...
    def log_images(self, batch, **kwargs):
        ...
        xrec, _, encoder_features, decoder_features, encoder_features_unfold, decoder_features_unfold, _ = self(x)
        if x.shape[1] > 3:
