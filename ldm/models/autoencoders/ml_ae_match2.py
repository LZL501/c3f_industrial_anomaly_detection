...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
        ...
        self.conv1 = torch.nn.Conv2d(in_channels, out_channels//2, kernel_size=3, stride=1,padding=1, bias=False)
        if temb_channels > 0:
            self.temb_proj = torch.nn.Linear(temb_channels, out_channels)
        self.norm2 = Normalize(out_channels//2)
        self.dropout = torch.nn.Dropout(dropout)
        self.conv2 = torch.nn.Conv2d(out_channels//2, out_channels, kernel_size=3, stride=1, padding=1, bias=False)
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                self.conv_shortcut = torch.nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1,padding=1, bias=False)
            else:
                self.nin_shortcut = torch.nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
...
class Bottleneck(nn.Module):
    def __init__(self, *, in_channels, mid_channels=None, out_channels=None, conv_shortcut=False, dropout, temb_channels=512):
        ...
        self.norm1 = Normalize(in_channels)
        self.conv1 = torch.nn.Conv2d(in_channels, mid_channels, kernel_size=1, stride=1,padding=0, bias=False)

        self.norm2 = Normalize(mid_channels)
        self.conv2 = torch.nn.Conv2d(mid_channels, mid_channels, kernel_size=3, stride=1, padding=1, bias=False)

        self.norm3 = Normalize(mid_channels)
        self.conv3 = torch.nn.Conv2d(mid_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                self.conv_shortcut = torch.nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1,padding=1, bias=False)
            else:
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), down_samples=(4,2,2,2), num_res_blocks=(2,2,2,2),
        ...
        self.conv_out = torch.nn.Conv2d(block_in, out_ch, kernel_size=3, stride=1, padding=1, bias=False)
...
class VQModel(pl.LightningModule):
    def __init__(self,
        ...
        self.folds = folds
        self.feature_embed = nn.ModuleList([nn.Embedding((64 // 2 ** i) ** 2, feature_dim) for i, feature_dim in enumerate(feature_dims)])
    ...
    def encode(self, x):
        ...
        ef_with_pe = [ef + em.weight.reshape(ef.shape[1:]).unsqueeze(0) for ef, em in zip(encoder_features, self.feature_embed)]
        poses = [self.get_pos_embed[i](ef).repeat(ef.shape[0], 1, 1, 1).to(ef.device) for i, ef in enumerate(encoder_features)]
        ef_with_pe = [torch.cat((ef, pos), dim=1) for ef, pos in zip(ef_with_pe, poses)]
    ...
    def forward(self, input):
        ...
        # pred_mask = self.discriminator(torch.cat((input, dec), dim=1), encoder_features, decoder_features)
        pred_mask = None

        return dec, diffs, encoder_features, decoder_features, quants, indices, pred_mask
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        xrec, qloss, encoder_features, decoder_features, quants, _, pred_mask = self(x)
        
        # normal_x_features = self.encoder(x)
        # preds = (F.interpolate(dissimilarity(quants[0], normal_x_features[0]), scale_factor=4, mode='bilinear') + \
        #             F.interpolate(dissimilarity(quants[1], normal_x_features[1]), scale_factor=8, mode='bilinear') + \
        #             F.interpolate(dissimilarity(quants[2], normal_x_features[2]), scale_factor=16, mode='bilinear')) / 3
        # pred_mask = self.discriminator(preds, encoder_features, decoder_features)
        ...
        if optimizer_idx == 0:
            ...
            # if batch_idx > 2500:
            # aeloss = aeloss + self.focal_loss(pred_mask, mask.unsqueeze(1))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        xrec, qloss, encoder_features, decoder_features, neighbor_features, _, preds = self(xs)
    ...
    def log_images(self, batch, **kwargs):
        ...
        xrec, _, encoder_features, decoder_features, _, _, preds = self(x)
        preds = (F.interpolate(dissimilarity(decoder_features[0], encoder_features[0]), scale_factor=4, mode='bilinear') + \
                    F.interpolate(dissimilarity(decoder_features[1], encoder_features[1]), scale_factor=8, mode='bilinear') + \
                    F.interpolate(dissimilarity(decoder_features[2], encoder_features[2]), scale_factor=16, mode='bilinear')) / 3
        # preds = self.discriminator(preds, encoder_features, decoder_features)
...
class EncoderDiscriminative(nn.Module):
    def __init__(self, in_channels, base_width):
        ...
        self.block3 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width*4, base_width*4, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(base_width*4),
            nn.ReLU(inplace=True))
        self.mp3 = nn.Sequential(nn.MaxPool2d(2))
        self.block4 = nn.Sequential(
            nn.Conv2d(base_width*4, base_width*8, kernel_size=3, padding=1, bias=False),
...
class DecoderDiscriminative(nn.Module):
    def __init__(self, base_width, out_channels=1):
        ...
        self.up2 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 nn.Conv2d(base_width * 8, base_width * 4, kernel_size=3, padding=1, bias=False),
                                 nn.BatchNorm2d(base_width * 4),
                                 nn.ReLU(inplace=True))
        ...
        self.up3 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 nn.Conv2d(base_width * 4, base_width * 2, kernel_size=3, padding=1, bias=False),
                                 nn.BatchNorm2d(base_width * 2),
                                 nn.ReLU(inplace=True))
        self.db3 = nn.Sequential(
            nn.Conv2d(base_width * (2+2), base_width * 2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(base_width * 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 2, base_width * 2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(base_width * 2),
        ...
        self.up4 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 nn.Conv2d(base_width * 2, base_width, kernel_size=3, padding=1, bias=False),
                                 nn.BatchNorm2d(base_width),
                                 nn.ReLU(inplace=True))
        self.db4 = nn.Sequential(
            nn.Conv2d(base_width * (1+1), base_width, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
