...
class VQModel(pl.LightningModule):
    def __init__(self,
        ...
        self.discriminator = DiscriminativeSubNetwork(in_channels=6, out_channels=2, base_channels=64, out_features=False)
    ...
    def encode(self, x):
        ...
        for i, ef in enumerate(ef_with_pe):
            ...
            qloss += vq_info[1]
            # cos_sim = (torch.cosine_similarity(efu, vq_info[0], dim=1).unsqueeze(1) + 1) / 2 / 2
            ...
            for j in range(int(np.log2(self.folds[i]))):
                ...
                qloss += vq_info[1]
                # cos_sim = (torch.cosine_similarity(mid_features, vq_info[0], dim=1).unsqueeze(1) + 1) / 2 / 2
    ...
    def forward(self, input):
        ...
        preds = F.interpolate(dissimilarity(decoder_features[0], encoder_features[0]), scale_factor=4, mode='bilinear') + \
                    ...
                    F.interpolate(dissimilarity(decoder_features[2], encoder_features[2]), scale_factor=16, mode='bilinear')   # [B, 1, H, W]
        
        # pred_mask = self.discriminator(torch.cat((input, dec), dim=1) * preds, encoder_features, decoder_features)
        pred_mask = self.discriminator(torch.cat((input, dec), dim=1), encoder_features, decoder_features)
        return dec, diffs, encoder_features, decoder_features, quants, pred_mask
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        xrec, qloss, encoder_features, decoder_features, quants, pred_mask = self(x_anomaly)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        xrec, qloss, encoder_features, decoder_features, quants, pred_mask = self(xs)
        preds = F.softmax(pred_mask, dim=1)[:, 1, :, :].cpu().numpy()
    ...
    def log_images(self, batch, **kwargs):
        ...
        xrec, _, encoder_features, decoder_features, _, pred_mask = self(x)
        if x.shape[1] > 3:
        ...
        if kwargs['split'] != 'train':
            ...
            result = torch.cat((result, heatmaps, (batch[1] * 2 - 1).unsqueeze(1).repeat(1, 3, 1, 1).detach().cpu()), dim=-2)
        else:
            preds = F.softmax(pred_mask, dim=1)[:, 1, :, :].cpu().numpy()
            _, _, h, w = xrec.shape
            heatmaps = torch.from_numpy(np.stack([cv2.applyColorMap(cv2.resize(normalize(p), (h, w)), cv2.COLORMAP_JET)[:, :, ::-1] for p in preds])).permute(0, 3, 1, 2) / 255 # [B, 3, H, W], 0~1
            heatmaps = (heatmaps * 0.3 + ((xrec.detach().cpu() + 1) / 2).cpu() * 0.5) * 2 - 1
            result = torch.cat((result, heatmaps, (batch[2] * 2 - 1).unsqueeze(1).repeat(1, 3, 1, 1).detach().cpu()), dim=-2)
