...
def validate(val_loader, model, device):
    ...
    with torch.no_grad():
        for i, (image, mask, label, bg) in enumerate(val_loader):
            ...
            # preds_2_p.append(
            #              upsample_4(torch.mean((neighbor_features[0] - encoder_features[0]) ** 2, dim=1, keepdim=True)).cpu()+
            #              upsample_8(torch.mean((neighbor_features[1] - encoder_features[1]) ** 2, dim=1, keepdim=True)).cpu()+
            #              upsample_16(torch.mean((neighbor_features[2] - encoder_features[2]) ** 2, dim=1, keepdim=True)).cpu())
...
if __name__ == "__main__":
    ...
    ckpt = torch.load("logs/2023-04-28T11-45-41_mvtec_screw/checkpoints/best.ckpt")
